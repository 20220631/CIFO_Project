...
def plot_fitness(valid_sets, avg_fitness_values, min_fitness_values, max_fitness_values):
    # Plot average fitness values
    plt.plot(valid_sets, avg_fitness_values, label='Average', color='green')
    # Plot max fitness values
    plt.plot(valid_sets, max_fitness_values, label='Max', color='grey')
    # Plot min fitness values
    plt.plot(valid_sets, min_fitness_values, label='Min', color='grey')

    # Fill the area between max and min with grey color
    plt.fill_between(valid_sets, min_fitness_values, max_fitness_values, color='grey', alpha=0.5)

    plt.xlabel('Valid Set Range')
    plt.ylabel('Fitness')
...
def evolution_process(num_runs=5):
    ...
    for valid_set in range(10, 51):
        best_fitness_values = []
        print(f"valid set range:{valid_set}")
        for _ in range(num_runs):
            pop = Population(size=94, optim="min", sol_size=len(data_), valid_set=range(valid_set), replacement=True)
    ...
    valid_sets = np.arange(10, 51)
    plot_fitness(valid_sets, avg_fitness_values, min_fitness_values, max_fitness_values)
